...
static void cmd_Test_00(udp_peer_t *peer, iap_frame_t *IAP_Data);
static void cmd_ReportIp_01(udp_peer_t *peer, iap_frame_t *IAP_Data);
static void cmd_ForceModifyIP_02(udp_peer_t *peer, iap_frame_t *IAP_Data);
static void cmd_ReportFirmwareStatus_03(udp_peer_t *peer, iap_frame_t *IAP_Data);
static void cmd_PrepareUpgrade_04(udp_peer_t *peer, iap_frame_t *IAP_Data);
static void cmd_SendUpgradePackage_05(udp_peer_t *peer, iap_frame_t *IAP_Data);
static void cmd_EnterRecoveryMode_06(udp_peer_t *peer, iap_frame_t *IAP_Data);
static void cmd_Restart_07(udp_peer_t *peer, iap_frame_t *IAP_Data);
 ...
 * @param  ReData: 数据
 */
static void cmd_SendReData(udp_peer_t *peer, uint32_t ReSeq, uint32_t ReCmd, uint32_t ReLen, uint32_t *ReData)
{
    ...
    {
        UDP_SendBroadcast(peer->port, (uint8_t *)&ReBuff, sizeof(iap_frame_t) + U8_LEN(ReLen) + sizeof(uint32_t)); // 发送广播
    }
    else
    {
        UDP_Send(&peer->addr, peer->port, (uint8_t *)&ReBuff, sizeof(iap_frame_t) + U8_LEN(ReLen) + sizeof(uint32_t)); // 发送单播
    }
}
...
static void cmd_ReportIp_01(udp_peer_t *peer, iap_frame_t *IAP_Data)
{
    ...
    cmd_SendReData(peer, IAP_Data->seq, rtn_cmd01, U32_LEN(sizeof(ReData)), ReData);
}
...
static void cmd_ForceModifyIP_02(udp_peer_t *peer, iap_frame_t *IAP_Data)
{
    ...
    cmd_SendReData(peer, IAP_Data->seq, rtn_cmd02, 0, NULL);
}
...
static void cmd_ReportFirmwareStatus_03(udp_peer_t *peer, iap_frame_t *IAP_Data)
{
    ...
    cmd_SendReData(peer, IAP_Data->seq, rtn_cmd03, U32_LEN(sizeof(ReData)), ReData);
}
...
static void cmd_EnterRecoveryMode_06(udp_peer_t *peer, iap_frame_t *IAP_Data)
{
    ...
    cmd_SendReData(peer, IAP_Data->seq, rtn_cmd06, 0, NULL);
}
...
static void cmd_Restart_07(udp_peer_t *peer, iap_frame_t *IAP_Data)
{
    cmd_SendReData(peer, IAP_Data->seq, rtn_cmd07, 0, NULL);
...
/**
 * @brief 读取config_info的网络信息写入mainAPP中的储存器
 *
 * @note 适用于38333024控制卡-储存器:w25q256
 */
void Git_info_To_mainAPP(NET_Parameter_t *param_tmp)
{
    SysInfo_t config_info = *((SysInfo_t *)ADDR_CONFIG_SECTOR);

    memcpy(param_tmp->IP, config_info.net_cfg.ip, 4);
    memcpy(param_tmp->NETMASK, config_info.net_cfg.mask, 4);
    memcpy(param_tmp->WG, config_info.net_cfg.gw, 4);
    param_tmp->PORT = config_info.net_cfg.port & 0xFFFF;
}

/**
 * @brief  获取mainAPP网络信息写入config_info
 *
 * @note 用于mainapp协议修改系统IP信息时，将修改后的信息写入config_info
 */

void Git_mianAPP_To_info(NET_Parameter_t *param)
{
    SysInfo_t config_info = *((SysInfo_t *)ADDR_CONFIG_SECTOR);

    memcpy(config_info.net_cfg.ip, param->IP, 4);
    memcpy(config_info.net_cfg.mask, param->NETMASK, 4);
    memcpy(config_info.net_cfg.gw, param->WG, 4);
    config_info.net_cfg.port = param->PORT & 0xFFFF;
